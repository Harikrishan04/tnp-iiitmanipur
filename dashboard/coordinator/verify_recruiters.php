            ...
            <main class="flex-1 flex flex-col min-h-0 p-4 md:p-8 w-full">
                <div class="flex flex-1 gap-6 min-h-0">
                    <div class="w-full md:w-1/3 bg-white rounded-lg shadow-lg flex flex-col p-6 min-h-0">
                        ...
                        <div id="recruiterList" class="flex-1 flex flex-col items-center justify-center text-gray-400 overflow-y-auto">
                    ...
                    <div class="w-full md:w-2/3 bg-white rounded-lg shadow-lg flex flex-col p-6 min-h-0">
                        <div id="recruiterDetails" class="flex flex-1 flex-col items-center justify-center text-gray-400 overflow-y-auto">
    ...
    async function handleMessageAction(action, recruiterId) {
        ...
        try {
            ...
            const response = await fetch(apiUrl, {
                ...
                body: `recruiter_id=${recruiterId}&status=${action === 'modify' ? 'modified' : action}&remark=${encodeURIComponent(message)}&coordinator_id='<?php echo $coordinatorId; ?>'`,
                credentials: 'include',
            });
