...
<head>
    ...
    <link href="../../assets/css/toastify.1.6.1.min.css" rel="stylesheet">
    <script src="../../assets/js/jquery.min.js"></script>
    <script src="../../assets/js/select2.4.1.0.min.js"></script>
    <script src="../../assets/js/toastify.1.6.1.min.js"></script>
    <style>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <div id="viewProfileModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl overflow-auto flex flex-col">
   ...
   <div id="updateProfileModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl overflow-auto flex flex-col">
        ...
        <div class="flex flex-1 min-h-0">
            <div  style="max-height: calc(90vh - 60px);" class="w-[90px] h-full overflow-y-auto bg-gray-50 p-2 flex-shrink-0" id="side-progress-bar">
                <div class="flex flex-col items-center py-4">
                </div>
            </div>
            <div style="max-height: calc(90vh - 60px);" class="flex-1 overflow-y-auto overflow-x-auto p-6"> <form method="POST" id="profile-form" class="flex flex-col min-h-full">
                    </form>
            </div>
        </div>
    </div>
        ...
        if(dataArray.length === 0) { // Add one empty entry if none exist
            ...
            dataArray.forEach(item => {
                ...
                Object.keys(item).forEach(key => {
                    let fieldName = key;
                    if(type === 'certificate' && key === 'name') fieldName = 'name';
                    if(type === 'certificate' && key === 'platform') fieldName = 'platform';
                    if(type === 'certificate' && key === 'year') fieldName = 'year';
                    if(type === 'projects' && key === 'title') fieldtitle = 'name';
                    if(type === 'projects' && key === 'link') fieldName = 'link';
                    if(type === 'projects' && key === 'description') fieldName = 'description';
                    if(type === 'internship' && key === 'duration')  fieldName = 'duration'; // Skip duration
        ...
        const content = {
            1: `
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div><label for="program" class="form-label">Program</label><input type="text" id="program" class="form-input"></div>
                    <div><label for="department" class="form-label">Department</label><input type="text" id="department" class="form-input"></div>
...
const studentId = "<?php echo $user_id ?? ''; ?>";
    ...
    // Ensure studentId is defined in the scope where this function is called.
    // For example: const studentId = 'YOUR_student_UUID';
    ...
    try {
        ...
        const res = await fetch(`../../dataRouting/api/getVerificationStatusById.php?verification_id=${studentId}`); // Changed event_id to verification_id to match PHP's expected parameter
        ...
        if (data.status === 'success' && data.data) { // Check for success and if 'data' property exists
            ...
            banner.innerHTML = `
                <div class="bg-red-100 border border-red-300 text-red-800 rounded-lg px-6 py-4 shadow flex items-center gap-4">
                    ...
                    <div>
                        <div class="font-semibold">Status: <span class="font-bold">Complete Profile</span></div>
